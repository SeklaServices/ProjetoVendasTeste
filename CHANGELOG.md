...
> **Versão major por quebra de contrato.** O `POST /api/v1/vendas` deixou de aceitar `cliente` e
> passou a exigir `clienteId`. Quem consumir a API precisa ajustar a chamada — ver *Alterado*.
...
- **⚠️ Contrato da API de vendas (breaking change)** — `POST /api/v1/vendas` passa a receber
  `clienteId` (o `Guid` de um cliente cadastrado e ativo) no lugar de `cliente` (texto livre). As
  respostas de `GET /api/v1/vendas` e `GET /api/v1/vendas/{id}` trocam o campo `cliente` por
  `clienteId`, `clienteCodigo` e `clienteNome`
...
[2.0.0]: https://github.com/SeklaServices/ProjetoVendasTeste/releases/tag/v2.0.0
